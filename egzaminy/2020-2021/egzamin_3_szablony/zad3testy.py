...
def tree2str(p):
  if p is not None:
    s = tree2str(p.left_child)
    s += " %d " % p.key 
    s += tree2str(p.right_child)
    return s
  else:
    return ""
...
def findplace(p,d):
    if p.left_child == None or p.right_child == None:
        return d, p
    g1, p1 = findplace(p.left_child, d + 1)
    g2, p2 = findplace(p.right_child, d + 1)
    if p1 == None:
        return g2, p2
    if p2 == None:
        return g1, p1
